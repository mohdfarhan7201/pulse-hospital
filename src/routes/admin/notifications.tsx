...
import {
  listAdminNotificationsFn,
  markNotificationReadFn,
  markAllAdminNotificationsReadFn,
} from "@/lib/api";
...
function AdminNotificationsPage() {
  ...
  const markAllReadMutation = useMutation({
    mutationFn: () => markAllAdminNotificationsReadFn(),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin-notifications"] });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-xl font-bold tracking-tight">Hospital Notifications</h2>
          <p className="text-sm text-muted-foreground">
            System alerts, doctor updates, and administrative activities.
          </p>
        </div>
        {unreadCount > 0 && (
          <Button
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              {filter === "unread"
                ...
                : "No hospital notifications to display right now."}
            </p>
          </div>
        )}
